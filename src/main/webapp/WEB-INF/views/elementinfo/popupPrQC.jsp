...
<script type="text/javascript">
	$(document).ready(function() {
		...
		const pr_qc_id = urlParams.get('pr_qc_id');
		const pr_num = urlParams.get('pr_num');
		...
		<table id="elementlist" class="table align-middle table-bordered table-hover mb-0">
		  ...
		  <tbody id="elementlistBody">
		  	<c:forEach var="vo" items="${eiList}">
		  		<tr>		  		
			  		<td><input type="checkbox" <c:if test="${vo.pr_qc_defect eq 1 }">checked</c:if>></td>
			  		<td>${vo.ele_num }</td>
		  		</tr>
		  	</c:forEach>
		  </tbody>
